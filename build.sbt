...
lazy val `zio-user-view` =
  project
    .in(file("."))
    .enablePlugins(GitVersioning)
    .aggregate(`core`, `user-view-api`, `user-view-svc`)
    .settings(settings)
    .settings(
...
lazy val `user-view-api` =
  (project in file("modules/user-view-api"))
    .settings(settings)
    .settings(
      addCompilerPlugin("org.typelevel" %% "kind-projector" % Versions.kindProjector cross CrossVersion.full),
      Compile / PB.targets := Seq(
...
lazy val `user-view-svc` =
  (project in file("modules/user-view-svc"))
    ...
    .aggregate(`user-view-api`)
    .dependsOn(`user-view-api`, `core`)
